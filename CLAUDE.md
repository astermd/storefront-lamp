...
- Baseline: `vendor/bin/phpunit` is green at **2202 tests / 7432 assertions**, both with a synced catalog and on a clone that has not synced one, where the suite falls back to `config/products.generated.example.php`. Both cases must stay green. The assertion count tracks the synced catalog's size, so it can differ on your channel — the test count should not, and a test that changes its own count when `theme:sync` runs is a test asserting what the last sync happened to contain. Anything red is yours — establish the baseline before you change anything, so a pre-existing failure is never mistaken for one you caused.
...
- **Test methods are camelCase** — `testTheThingItAsserts()`. 2104 of them, zero snake_case. There are no `#[Test]` attributes; the `test` prefix is what PHPUnit discovers, so a renamed method silently stops running.
