...
| Theme watch | `npm run dev`     | rebuilds on save and reloads the page; for design work, never for a commit |
...
- Baseline: `vendor/bin/phpunit` is green at **2215 tests**, at 7524 assertions against a synced catalog and 7465 on a clone that has not synced one, where the suite falls back to `config/products.generated.example.php`. Both cases must stay green. The assertion count tracks the synced catalog's size, so it can differ on your channel — the test count should not, and a test that changes its own count when `theme:sync` runs is a test asserting what the last sync happened to contain. Anything red is yours — establish the baseline before you change anything, so a pre-existing failure is never mistaken for one you caused.
...
- **Everything in `core/` is `final`** — all 227 classes, no exceptions (the four enums are implicitly final). Compose and inject instead of subclassing, so behaviour stays a wiring decision in `core/Bootstrap/AppFactory.php` rather than an inheritance graph nobody can see the shape of.
...
- **Test methods are camelCase** — `testTheThingItAsserts()`. 2117 of them, zero snake_case. There are no `#[Test]` attributes; the `test` prefix is what PHPUnit discovers, so a renamed method silently stops running.
...
- **`npm run dev` is the designer's loop; `npm run build` is what gets committed.** The watcher (`build/watch.mjs`) re-runs the same build on every save under `theme/css`, `theme/js` and `theme/templates`, and `theme/js/livereload.js` — loaded by `layouts/base.twig` only when `app.env` is not `production` — polls the manifest and reloads the page. Finish with a plain `npm run build` so the committed hashes match the sources.
