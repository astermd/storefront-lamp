 ...
 *
 * The counterpart to {@see \AsterMD\Storefront\Payment\Vrio\VrioWireLog}, and
 * it costs strictly more than that one does.
 *
 * **This provider takes every parameter in the query string**, including the
 * card number, the security code and the account password. So while this is on,
 * the transcript holds cardholder data *and this deployment's provider
 * credentials* in clear text, in the URL rather than in a body -- and a URL is
 * the part of a request that the most things copy: proxies, gateways, APM
 * agents and crash reporters all record one by default.
 *
 * That is `[15.8]`'s prohibition and a credential leak in the same file, which
 * is why the guard rails are the same three and are worth restating:
 *
 ...
 *
 * A deployment that switches this on should treat the resulting file as a
 * secret to be destroyed, not as a log to be shipped to an aggregator.
 *
 ...
 */
final class CheckoutChampWireLog implements HttpClientInterface
{
     ...
     * One line per call.
     *
     * Failures are swallowed for the reason every other sink's are: a debugging
     * aid that could take a checkout down would be a worse bug than whatever it
