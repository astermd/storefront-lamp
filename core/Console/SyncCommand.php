 ...
 *   both are overwritten by the fresh payload. **Two exceptions, both about
 *   not letting one channel's data outlive it:** the `payment_processor`
 *   block is replaced wholesale on every sync, and a change of `channel.id`
 *   replaces the whole file. {@see self::mergeOntoExisting()} for why.
 *
...
final class SyncCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $channelHeader = "GENERATED by bin/console theme:sync — do not hand-edit.\n"
            . "Hand-added keys outside payment_processor survive a re-sync of the same channel.\n"
            . "payment_processor is replaced every sync, and a different channel replaces this file.\n"
        ...
        $mergedChannelPayload = self::mergeOntoExisting($existingChannelPayload, $channelPayload);
    ...
    /**
     * The merged channel file — with two things the merge does **not** get to
     * keep.
     *
     * Merge-keep-extra exists so a hand-added operational key survives a
     * re-sync, and that is worth keeping for the file as a whole. What it
     * cannot see is *who wrote* an existing key, so it treats the previous
     * provider's own credentials exactly like an operator's hand-addition.
     *
     * Pointing `.env` at a channel on a different payment processor and
     * re-syncing therefore produces a `payment_processor.config` holding the new
     * provider's credentials **beside the previous one's live API key**, plus
     * its routing hints. Two providers' secrets in one file, and a set of keys
     * meaning nothing to the provider now configured — in the block
     * `AdapterRegistry`'s chosen adapter reads its credentials from.
     *
     * So:
     *
     * - **A different `channel.id` replaces the file wholesale.** This is a
     *   different deployment target and nothing in the old file describes it.
     * - **`payment_processor` is replaced wholesale on every sync.** It is
     *   generated data end to end: every key in it is one provider's own
     *   vocabulary, written by the EMR, and nothing in this codebase reads a
     *   hand-added key from it — `payment.shipping_profile_id` is read from
     *   `config/payment.php`, and the wire log switch from `app.debug.wire_log`.
     *   Merging it can therefore only ever preserve a key the configured
     *   provider has no use for, and at worst a credential belonging to a
     *   provider this deployment no longer talks to.
     *
     * Everything outside that block still merges, so an operator's own key
     * elsewhere in the tree survives a re-sync of the same channel.
     *
     * @param array<string, mixed> $existing
     * @param array<string, mixed> $new
     * @return array<string, mixed>
     */
    private static function mergeOntoExisting(array $existing, array $new): array
    {
        if ($existing === [] || self::identity($existing) !== self::identity($new)) {
            return $new;
        }

        $merged = self::mergeKeepExtra($existing, $new);
        $merged['payment_processor'] = $new['payment_processor'] ?? [];

        return $merged;
    }

    /**
     * The channel this file describes, as a comparable string.
     *
     * An absent id answers `''` rather than null so that "this file predates
     * the key" and "this file says it is empty" compare equal — neither is
     * evidence of a *change*, and reading a missing key as a difference would
     * discard an operator's hand-added key on the first sync after an upgrade.
     *
